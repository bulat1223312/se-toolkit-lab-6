...
# The system attempts to source configuration parameters from a prioritized list of environment files,
# ensuring that secrets and runtime settings are correctly initialized.
...
# LLM configuration
# These variables define the endpoint, authentication credentials, and model identifier
# for the large language model service utilized by the agent.
API_KEY = os.getenv("LLM_API_KEY")
API_BASE = os.getenv("LLM_API_BASE")
MODEL = os.getenv("LLM_MODEL", "qwen3-coder-plus")

# Backend API configuration
# The agent communicates with the backend Learning Management System (LMS) API
# using the base URL and an optional authorization key.
LMS_API_KEY = os.getenv("LMS_API_KEY", "")
AGENT_API_BASE_URL = os.getenv("AGENT_API_BASE_URL", "http://localhost:42002")

# Project root for security checks
# This absolute path serves as the baseline for validating file access requests,
# preventing directory traversal attacks by ensuring all file operations remain within the project boundaries.
PROJECT_ROOT = Path(__file__).parent.resolve()

# Maximum tool calls per question
# To avoid infinite loops and excessive resource consumption, the agent is constrained
# to a finite number of tool invocations while resolving a single user query.
MAX_TOOL_CALLS = 10

# System prompt for the system agent
# This comprehensive instruction set informs the language model about its role,
# the available tools, the expected interaction protocol, and the precise JSON format for responses.
...
def is_safe_path(path: str) -> bool:
    ...
    This function validates that the provided filesystem path does not contain
    directory traversal sequences (e.g., '..') and is not absolute, thereby
    mitigating the risk of unauthorized access outside the designated project root.
    """
...
def read_file(path: str) -> str:
    ...
    After verifying the path's safety, the function attempts to read the file's content
    as UTF-8 encoded text. In the event of a missing file, non-file entity, or any I/O error,
...
def list_files(path: str) -> str:
    ...
    This function enumerates the contents of a directory specified by the input path,
    subject to safety constraints. The returned string comprises a newline-separated
    listing of the directory entries, sorted alphabetically.
    """
    if not is_safe_path(path):
        return "Error: Access denied - invalid path"
...
def extract_json_from_response(content: str) -> dict | None:
    ...
    # This straightforward approach is attempted initially for efficiency.
    try:
    ...
    # This algorithm scans the string character by character, maintaining a depth counter
    # and correctly handling escaped characters and quoted strings to identify the outermost JSON object.
    ...
    # As a last resort, a regular expression captures the simplest possible JSON structure,
    # though this may be insufficient for nested objects.
...
def run_agent(question: str) -> dict:
    ...
    for iteration in range(MAX_TOOL_CALLS):
        ...
        if parsed is None:
            ...
            # If the model's output is not valid JSON, we assume it is the final textual answer.
        ...
        if "tool_call" in parsed:
            ...
            # The tool invocation and its outcome are appended to the conversation history
            # to provide the LLM with the necessary context for subsequent reasoning steps.
            ...
            # This heuristic attempts to assign a source document when the LLM omitted it,
            # preferring the most recently read file as the probable origin.
            ...
            # Any JSON object that does not conform to the expected schema is interpreted
            # as a final answer, and the source is derived from the last read_file call if available.
            answer = response_content
            if tool_calls_log:
                for tc in reversed(tool_calls_log):
                    if tc["tool"] == "read_file":
